...
uses
  ...
  Dialogs, sTooledit, Buttons, sSpeedButton,
  Mask, sCurrEdit, sCurrencyEdit, ExtCtrls,
  ...
  dxSkinscxPCPainter, cxNavigator, cxFilter, cxData, sMaskEdit, sCustomComboEdit;
...
begin
  ...
  begin
    ...
    begin
      ...
      harga := dm.Q_temp.fieldbyname('harga_pokok').AsFloat / dm.Q_temp.FieldByName
        ('qty_receipt').AsFloat;
...
begin
  ...
  try
    fungsi.SQLExec(dm.Q_exe,
      ...
      ed_tgl.Date) + '","' + ed_supplier.Text + '",7,"' + tunai + '","' +
      plus_PPN + '",0,0,"' + ed_nilai_faktur.Text + '","' + dm.kd_pengguna +
      '",now())', false);
