...
begin
if not DirectoryExists(dm.a_path+'DATA_KIRIM') then
  MkDir(dm.a_path+'DATA_KIRIM');
...
begin
if not DirectoryExists(dm.a_path+'DATA_TERIMA') then
  MkDir(dm.a_path+'DATA_TERIMA');

dir_load:= dm.a_path+'DATA_TERIMA\';
file_load:=dm.a_path+'DATA_TERIMA\PC_'+f_utama.sb.Panels[5].text+'_'+formatdatetime('yyyy-MM-dd',edt_terima.Date)+'.zip';
end;
...
zipp.AutoSave:=True;
zipp.BaseDirectory:= dm.a_path+'DATA_KIRIM\';
